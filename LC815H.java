...
public class LC815H {
    public int numBusesToDestination(int[][] routes, int source, int target) {
    	if(source==target)
    		return 0;
    	int ans=0,out = Integer.MAX_VALUE;
    	...
    	for(int i=0;i<routes.length;i++) {
    		for(int j=0;j<routes[i].length;j++) {
    			if(hash.containsKey(routes[i][j])) {
    				hash.get(routes[i][j]).add(i);
    			}else {
    	    		List<Integer> l = new LinkedList<>();
    	    		l.add(i);
        			hash.put(routes[i][j], l);
    			}
    		}
    	}
    	boolean[][] adj = new boolean[routes.length][routes.length];
    	for(int i=0;i<routes.length;i++) {
    		for(int site:routes[i]) {
    			if(hash.containsKey(site)) {
    	    		List<Integer> l = hash.get(site);//公交集合
    	    		//将公交对应的站点添加到队列
    	    		for(int n:l) {
    	    			if((n==1&&i==2)||(n==2&&i==1))
    	    				n=n;
    	...
    	//bfs
    	List<Integer> start = hash.get(source);//起点公交站
    	List<Integer> end = hash.get(target);//终点公交站
		boolean Find = false;
    	int[] visited = new int[(int) Math.pow(10, 6)];
...
//    	            	out = Math.min(out, visited[temp]);
//    	            	Find = true;
//    	            	break;
//    	    		}
    	    		
    	    		boolean[] l = adj[temp];//公交集合
    	    		//将公交对应的站点添加到队列
...
//		set.add(1);
		...
		System.out.println(lc.numBusesToDestination(routes, source, target));
